...
Movie.prototype.changeRating = function (newRating) {
  let average = 0;
  average = (this.rating + newRating) / 2;
  this.rating = average;
  return this.rating;
};
...
User.prototype.removeSavedPost = function (postId) {
  this.savedPosts = this.savedPosts.filter((element) => element.id !== postId);
...
User.prototype.changePostRating = function (id, newRating) {
  /////// WITH FIND INDEX ////////
  // const indexToChange = this.savedPosts.findIndex(
  //   (element) => element.id === id
  // );
  // this.savedPosts[indexToChange].rating = newRating;

  ////// WITH A MAP FUNCTION ////
  this.savedPosts = this.savedPosts.map((e) =>
    e.id === id ? { ...e, rating: newRating } : e
  );
  return this.savedPosts;
};
